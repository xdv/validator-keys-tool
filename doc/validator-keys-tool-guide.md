...
change if the divvyd config and/or server are compromised.
...
*   sign tokens authorizing a divvyd server to run as the validator identified
    by this public key.
...
The current token needs to be present in the divvyd config file.
...
Sample output:
```
  Validator keys stored in /home/ubuntu/.divvy/validator-keys.json
```
...
For a new validator, add the [validator_token] value to the divvyd config file.
...
value. After the config is updated, restart divvyd.
...
```
  ...
  Update divvyd.cfg file with these values and restart divvyd:
...
restart divvyd. Rename the old key file and generate new [validator keys](#validator-keys) and
